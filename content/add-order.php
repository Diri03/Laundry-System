...
// <td><input type="number" step="any" class="form-control" name="qty[]" placeholder="Enter your quantity"></td>
    ...
    if (isset($_POST["save"])) {
        ...
        if ($insert) {
            ...
            for ($i=0; $i < count($_POST['id_service']); $i++) { 
                ...
                mysqli_query($conn, "INSERT INTO trans_order_detail (id_order, id_service, qty, notes, subtotal) VALUES ('$id_order', '$id_service', '$qty', '$notes', '$subtotal')");
            }
            $updateTotal = mysqli_query($conn, "UPDATE trans_order SET total='$hitungTotal' WHERE id='$id_order'");
    ...
    if (isset($_POST['save2'])) {
        ...
        $update = mysqli_query($conn, "UPDATE trans_order SET order_status='$order_status', order_pay='$order_pay', order_change='$order_change' WHERE id='$id_order'");
        if ($update) {
